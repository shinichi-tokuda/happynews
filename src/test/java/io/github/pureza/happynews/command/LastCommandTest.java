...
public class LastCommandTest extends AbstractTest {
    ...
    public void failWhenUserIsNotWithinGroup() throws Exception {
        ...
        new LastCommand(user, "LAST", server).process();
    ...
    public void failWhenCurrentArticleIsTheFirst() throws Exception {
        ...
        assertThat(out.get(), containsString("422 no previous article in this group"));
        assertThat(user.getCurrentGroup(), equalTo(group));
    }


    @Test
    public void movesToPreviousArticleOnSuccess() throws Exception {
        Newsgroup group = new Newsgroup("group", date(2004, 10, 3, 10, 20), asList("<1@host>", "<2@host>"));

        when(server.getArticle("<1@host>")).thenReturn(new Article("<1@host>", config.articlesHome()));
        ...
        assertThat(out.get(), containsString("223 1 <1@host> article retrieved - request text separately"));
        assertThat(user.getCurrentArticleIndex(), equalTo(1));
    }
}